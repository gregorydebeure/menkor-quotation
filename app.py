...
_AP = {}  # Loaded at startup from airports.json

def _load_airports():
    """Load airport database from JSON file."""
    global _AP
    import json, os
    # Try loading from same directory as this script
    paths = [
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "airports.json"),
        "airports.json",
    ]
    for path in paths:
        if os.path.exists(path):
            ...
            _AP = data
            return
    # Fallback: minimal hardcoded dict if file missing
    _AP = {
        "OMDB":["Dubai",25.2528,55.3644],"DXB":["Dubai",25.2528,55.3644],
        "LFPB":["Paris Le Bourget",48.9694,2.4414],"EGLL":["London Heathrow",51.4775,-0.4614],
        "LFPG":["Paris CDG",49.0097,2.5479],"LHR":["London Heathrow",51.4775,-0.4614],
    }

_load_airports()

def geocode(place):
    """Look up airport by ICAO, IATA or city name. 100% offline."""
    if not _AP:
        _load_airports()
    k = place.strip().upper().replace(" ","").replace("-","")
    # Direct key match
    if k in _AP:
        v = _AP[k]; return v[1], v[2], v[0]
    # ICAO/IATA exact
    if place.strip().upper() in _AP:
        v = _AP[place.strip().upper()]; return v[1], v[2], v[0]
    # Partial key match
    for key, v in _AP.items():
        if k in key or key in k:
            return v[1], v[2], v[0]
    # City/name match
    pl = place.strip().lower()
    for key, v in _AP.items():
        if pl in v[0].lower() or v[0].lower().startswith(pl):
            return v[1], v[2], v[0]
